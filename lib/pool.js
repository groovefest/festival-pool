...
function processShare(miner, job, blockTemplate, nonce, resultHash){
    ...
    if (shareTrustEnabled && miner.trust.threshold <= 0 && miner.trust.penalty <= 0 && Math.random() > miner.trust.probability){
        ...
        var majorVersion = template[0];
        if (majorVersion >= 4) {
            // Arto Variant
            hash = cnUtil.cn_slowhash_variant(convertedBlob, 4);
        }
        else {
            // Standard Cryptonight
            hash = cnUtil.cn_slowhash_variant(convertedBlob, 0);
        }
        console.log("ProcessShare Verify - Blob:", convertedBlob.toString('hex').substr(0, 10), "Hash:", hash.toString('hex'), "Result:", resultHash, "majorVersion:", majorVersion);
